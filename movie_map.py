...
def process_data(option, args):
    ...
    '''
    try:
        num_of_movies_to_process = input("enter a number of movies that you "
                                        "want to process: ")
        num_of_movies_to_process = int(num_of_movies_to_process)
    except ValueError:
        print("You must to enter an integer!")
        process_data(option, args)
    line_num = 0
    data_to_mark = dict()
    with open("locations.list.txt", "r", errors="ignore") as text:
        for line in text.readlines()[:num_of_movies_to_process]:
            line_num += 1
            if line_num > 14:
                try:
